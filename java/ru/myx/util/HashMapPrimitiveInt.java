...
public class HashMapPrimitiveInt<V> implements Serializable {
	 ...
	 * 
	 */
	private static final long	serialVersionUID			= -2672503595759495534L;
	 ...
	 */
	public HashMapPrimitiveInt(final int initialCapacity) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public HashMapPrimitiveInt(int initialCapacity, final float loadFactor) {
	 ...
	 * @return
	 */
	public Collection<HashMapPrimitiveInt.Entry<V>> entrySet() {
		final List<HashMapPrimitiveInt.Entry<V>> result = new ArrayList<>( this.size );
		final Entry<V>[] tab = this.table;
		for (final Entry<V> element : tab) {
			for (Entry<V> e = element; e != null; e = e.next) {
				result.add( e );
	...
	public final V remove(final int key) {
		final int hash = HashMapPrimitiveInt.hash( key );
		final Entry<V> e = this.removeEntryForKey( hash, key );
		return e == null
				? null
				: e.value;
	}
