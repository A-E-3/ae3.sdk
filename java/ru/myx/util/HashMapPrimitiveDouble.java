...
class HashMapPrimitiveDouble<V> {
	...
	static final class Entry<V> {
		 ...
		 * Create new entry.
		 */
		Entry(final int h, final double k, final V v, final Entry<V> n) {
			this.value = v;
			this.next = n;
			this.key = k;
			this.hash = h;
		...
		public final boolean equals(final Object o) {
			...
			final double k1 = this.key;
			final double k2 = e.key;
			if (k1 == k2) {
				final V v1 = this.getValue();
				final V v2 = e.getValue();
		...
		public final int hashCode() {
			final long bits = Double.doubleToLongBits( this.key );
	...
	private static int hash(final double key) {
		final long bits = Double.doubleToLongBits( key );
		return HashMapPrimitiveDouble.hash( (int) (bits ^ bits >>> 32) );
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	HashMapPrimitiveDouble() {
		this.loadFactor = HashMapPrimitiveDouble.DEFAULT_LOAD_FACTOR;
		this.threshold = (int) (HashMapPrimitiveDouble.DEFAULT_INITIAL_CAPACITY * HashMapPrimitiveDouble.DEFAULT_LOAD_FACTOR);
		this.table = new Entry[HashMapPrimitiveDouble.DEFAULT_INITIAL_CAPACITY];
	}
	 ...
	 */
	HashMapPrimitiveDouble(final int initialCapacity) {
		this( initialCapacity, HashMapPrimitiveDouble.DEFAULT_LOAD_FACTOR );
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	HashMapPrimitiveDouble(int initialCapacity, final float loadFactor) {
		if (initialCapacity < 0) {
			throw new IllegalArgumentException( "Illegal initial capacity: " + initialCapacity );
		}
		if (initialCapacity > HashMapPrimitiveDouble.MAXIMUM_CAPACITY) {
			initialCapacity = HashMapPrimitiveDouble.MAXIMUM_CAPACITY;
		}
	 ...
	 *         key.
	 */
	public final boolean containsKey(final double key) {
		final int hash = HashMapPrimitiveDouble.hash( key );
	 ...
	 */
	public final V get(final double key) {
		final int hash = HashMapPrimitiveDouble.hash( key );
		for (Entry<V> e = this.table[hash & this.table.length - 1]; e != null; e = e.next) {
	 ...
	 */
	public final V put(final double key, final V value) {
		final int hash = HashMapPrimitiveDouble.hash( key );
		final int i = hash & this.table.length - 1;
		...
		this.table[i] = new Entry<>( hash, key, value, e );
	 ...
	 *         the specified key.
	 */
	public final V remove(final double key) {
	 ...
	 */
	private final Entry<V> removeEntryForKey(final double key) {
		final int hash = HashMapPrimitiveDouble.hash( key );
		final int i = hash & this.table.length - 1;
	...
	private final void resize(final int newCapacity) {
		...
		if (oldCapacity == HashMapPrimitiveDouble.MAXIMUM_CAPACITY) {
	...
	private void transfer(final Entry<V>[] newTable) {
		...
		for (int j = 0; j < src.length; j++) {
			...
			if (e != null) {
				...
				do {
					final Entry<V> next = e.next;
					final int i = e.hash & newCapacity - 1;
					e.next = newTable[i];
					newTable[i] = e;
					e = next;
				} while (e != null);
