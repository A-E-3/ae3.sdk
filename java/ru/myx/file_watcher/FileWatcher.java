...
public final class FileWatcher {
	private static final FileFilter	DEFAULT_FILTER	= new FilterDefault();
	...
	public static final synchronized boolean registerFile(final String appName, final Function<File, Object> target) {
		FileWatcher.CONTROLLER.add( new RecordFile( new File( FileWatcher.FOLDER_IMPORT, appName.toLowerCase()
				.replace( '\\', '_' ).replace( '/', '_' ).replace( ':', '_' ) ), FileWatcher.DEFAULT_FILTER, target ) );
		return true;
	}
	 ...
	 * @return boolean
	 */
	public static final boolean registerInputStream(
			final String appName,
			final Function<FileInputStream, Object> target) {
		FileWatcher.CONTROLLER.add( new RecordInputStream( new File( FileWatcher.FOLDER_IMPORT, appName.toLowerCase()
				.replace( '\\', '_' ).replace( '/', '_' ).replace( ':', '_' ) ), FileWatcher.DEFAULT_FILTER, target ) );
		return true;
	}
	...
	public static final boolean registerReader(final String appName, final Function<FileReader, Object> target) {
		FileWatcher.CONTROLLER.add( new RecordReader( new File( FileWatcher.FOLDER_IMPORT, appName.toLowerCase()
				.replace( '\\', '_' ).replace( '/', '_' ).replace( ':', '_' ) ), FileWatcher.DEFAULT_FILTER, target ) );
		return true;
	}
}
