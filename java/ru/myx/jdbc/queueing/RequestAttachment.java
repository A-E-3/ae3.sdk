 ...
 * 
 */
public abstract class RequestAttachment<T, C extends RunnerDatabaseRequestor> implements Function<C, T>, Holder<T>,
		FutureValue<T> {
	...
	/**
	 * If key is not NULL - runner will try not to execute same request more
	 * than once in one loop.
	 * 
	...
	public final RequestAttachment<T, C> getReference() {
		...
		if (this.loadResult == null) {
			throw new DatabaseWaitTimeoutException( "Wait timeout (hash=" + System.identityHashCode( this ) + ")!",
					this );
		}
