...
final class TokenStatementConst extends TokenStatementAbstract {
	
	private TokenStatementSingle	token;
	
	TokenStatementConst(final String identity, final int line) {
		super( identity, line );
	}
	...
	public final TokenStatement createStatement(final String identity, final int line) {
		return new TokenStatementConst( identity, line );
	}
	...
	public boolean setControlBreakUsed() {
		throw new UnsupportedOperationException( "No statments allowed within 'const' statement!" );
	}
	...
	public final String toString() {
		return this.token == null
				? "const"
				: "const " + this.token.toString().trim();
	}
}
