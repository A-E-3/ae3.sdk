...
final class TokenStatementRootOutput extends TokenStatementAbstract {
	...
	TokenStatementRootOutput(final String identity, final int line) {
		super(identity, line);
	}
	...
	public final void toAssembly(final ProgramAssembly assembly, final int startOffset) throws Exception {
		...
		final ProgramPart block = assembly.toProgram(size);
		{
			...
			if (this.locals == null) {
				assembly.addInstruction(OperationsA01.XEENTRCTRL_P.instruction(block.length(), ResultHandler.FA_BNN_NXT));
			} else {
				final BaseObject locals = TokenStatementAbstract.toLocalsObjectOrArray(this.locals);
				if (locals == null) {
					assembly.addInstruction(OperationsA01.XEENTRVARS_P.instruction(block.length(), ResultHandler.FA_BNN_NXT));
				} else {
					assembly.addInstruction(OperationsA01.XEENTRVARS_P.instruction(1 + block.length(), ResultHandler.FA_BNN_NXT));
