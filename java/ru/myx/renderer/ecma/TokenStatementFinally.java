...
final class TokenStatementFinally extends TokenStatementAbstract {
	TokenStatementFinally(final String identity, final int line) {
		super( identity, line );
	}
	...
	public final TokenStatement createStatement(final String identity, final int line) {
		return new TokenStatementFinally( identity, line );
	}
	...
	public final void toAssembly(final ProgramAssembly assembly, final int startOffset) throws Exception {
		this.addDebug( assembly, "finally{" );
		assembly.addError( "finally without try!" );
	}
}
