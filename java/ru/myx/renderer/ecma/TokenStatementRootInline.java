...
final class TokenStatementRootInline extends TokenStatementAbstract {
	...
	TokenStatementRootInline(final String identity, final int line) {
		super(identity, line);
	}
	...
	public final boolean addStatement(final TokenStatement statement) {
		...
		if (statement.isEmpty()) {
			/**
			 * TODO remove 'if' (with no regression: this one is saying EMPTY in
			 * initial state but the state changes then (when it's too late))
			 */
