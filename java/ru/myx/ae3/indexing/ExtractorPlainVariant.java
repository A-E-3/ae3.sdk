...
public class ExtractorPlainVariant {
	...
	public static final Collection<String> extractContent(final Collection<String> result, final String text) {
		...
		if ((state == ExtractorPlainVariant.ST_CHARS
				|| state == ExtractorPlainVariant.ST_CHARS_UPPER
				|| state == ExtractorPlainVariant.ST_COMPOUND || state == ExtractorPlainVariant.ST_COMPOUND_UPPER)
				&& token.length() > 0) {
			result.add( token.toString() );
		}
		...
		if ((state == ExtractorPlainVariant.ST_COMPOUND_CONTINUE || state == ExtractorPlainVariant.ST_COMPOUND_UPPER_CONTINUE)
				&& compound.length() > 0) {
			result.add( compound.substring( 0, compound.length() - 1 ) );
		}
		return result;
