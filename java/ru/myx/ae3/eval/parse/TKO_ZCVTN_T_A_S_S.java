...
final class TKO_ZCVTN_T_A_S_S extends TokenOperator {
	
	public static final TokenOperator INSTANCE = new TKO_ZCVTN_T_A_S_S();

	private TKO_ZCVTN_T_A_S_S() {
		//
	}
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (argumentA == ModifierArguments.AA0RB) {
			if (store == ResultHandler.FA_BNN_NXT) {
				assembly.addInstruction(Instructions.INSTR_CVTN_T_1_R_NN_NEXT);
				return;
			}
		...
		assembly.addInstruction(OperationsA10.ZCVTN_T.instruction(argumentA, 0, store));
	}

	@Override
	public final String toCode() {
		
		return OperationsA10.ZCVTN_T + "\t1\tS  ->S;";
	}

	@Override
	public TokenInstruction toExecDetachableResult() {
		
		return TKO_ZCVTN_D_A_S_S.INSTANCE;
	}
	...
	public TokenInstruction toStackValue(final ProgramAssembly assembly, final TokenInstruction argumentA, final boolean sideEffectsOnly) {
		...
		return new TKV_ZCVTN(argumentA, 0);
	}
}
