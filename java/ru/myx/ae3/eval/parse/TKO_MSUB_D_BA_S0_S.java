 ...
 *
 */
final class TKO_MSUB_D_BA_S0_S extends TokenOperator {

	public static final TokenOperator INSTANCE = new TKO_MSUB_D_BA_S0_S();

	private TKO_MSUB_D_BA_S0_S() {
		//
	}
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (store == ResultHandler.FA_BNN_NXT) {
			if (argumentA == ModifierArguments.AA0RB) {
				assembly.addInstruction(Instructions.INSTR_MSUB_D_BA_R0_V);
				return;
			}
			if (argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MSUB_D_BA_S0_V);
				return;
			}
		} else //
		...
		assembly.addInstruction(OperationsS2X.VMSUB_D.instruction(ModifierArgumentA30IMM.ZERO, argumentA, 0, store));
	}

	@Override
	public final String toCode() {

		return OperationsS2X.VMSUB_D + "\t2\tCS ->S\tCONST(0);";
	}
