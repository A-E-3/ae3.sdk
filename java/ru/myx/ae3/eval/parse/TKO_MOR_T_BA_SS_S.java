...
final class TKO_MOR_T_BA_SS_S extends TokenOperator {

	public static final TokenOperator INSTANCE = new TKO_MOR_T_BA_SS_S();

	private TKO_MOR_T_BA_SS_S() {
		// ignore
	}
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (store == ResultHandler.FA_BNN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MOR_T_BA_SS_V);
				return;
			}
		} else //
		if (store == ResultHandler.FB_BSN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MOR_D_BA_SS_S);
				return;
			}
		}

		assembly.addInstruction(OperationsA2X.XMBOR_T.instruction(argumentA, argumentB, 0, store));
	}

	@Override
	public final String toCode() {

		return OperationsA2X.XMBOR_T + "\t2\tSS ->S;";
	}

	@Override
	public final TokenInstruction toExecDetachableResult() {

		return TKO_MOR_D_BA_SS_S.INSTANCE;
	}

	@Override
	public final TokenInstruction toExecNativeResult() {

		return TKO_MOR_N_BA_SS_S.INSTANCE;
	}
}
