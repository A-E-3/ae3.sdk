...
final class TKO_ZCVTS_N_A_S_S extends TokenOperator {
	
	public static final TokenOperator INSTANCE = new TKO_ZCVTS_N_A_S_S();

	private TKO_ZCVTS_N_A_S_S() {
		//
	}
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (argumentA == ModifierArguments.AA0RB) {
			if (store == ResultHandler.FA_BNN_NXT) {
				assembly.addInstruction(Instructions.INSTR_CVTS_N_1_R_NN_NEXT);
				return;
			}
			if (store == ResultHandler.FB_BSN_NXT) {
				assembly.addInstruction(Instructions.INSTR_CVTS_N_1_R_SN_NEXT);
				return;
			}
		}
		if (argumentA == ModifierArguments.AE21POP) {
			if (store == ResultHandler.FB_BSN_NXT) {
				assembly.addInstruction(Instructions.INSTR_CVTS_N_1_S_SN_NEXT);
				return;
			}
		}
		if (store == ResultHandler.FA_BNN_NXT) {
			assembly.addInstruction(OperationsS10.VCVTS_N.instruction(argumentA, 0, ResultHandler.FA_BNN_NXT));
			return;
		}

		assembly.addInstruction(OperationsS10.VCVTS_N.instruction(argumentA, 0, store));
	}

	@Override
	public final String toCode() {
		
		return OperationsS10.VCVTS_N + "\t1\tS  ->S;";
	}
	...
	public TokenInstruction toStackValue(final ProgramAssembly assembly, final TokenInstruction argumentA, final boolean sideEffectsOnly) {
		...
		return new TKV_ZCVTS(argumentA, 2);
	}
}
