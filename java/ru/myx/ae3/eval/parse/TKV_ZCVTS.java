...
final class TKV_ZCVTS extends TokenValue {
	
	private final TokenInstruction argumentA;

	private int visibility;

	TKV_ZCVTS(final TokenInstruction argumentA, final int visibility) {
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (modifierA == ModifierArguments.AA0RB) {
			...
			if (store == ResultHandler.FA_BNN_NXT) {
				assembly.addInstruction(this.visibility == 2
					? Instructions.INSTR_CVTS_N_1_R_NN_NEXT
					: this.visibility == 1
						? Instructions.INSTR_CVTS_D_1_R_NN_NEXT
						: Instructions.INSTR_CVTS_T_1_R_NN_NEXT);
				return;
			}
			if (store == ResultHandler.FB_BSN_NXT) {
				assembly.addInstruction(this.visibility == 2
					? Instructions.INSTR_CVTS_N_1_R_SN_NEXT
					: Instructions.INSTR_CVTS_D_1_R_SN_NEXT);
				return;
			}
		} else //
		...
		assembly.addInstruction((this.visibility == 2
			? OperationsS10.VCVTS_N
			: this.visibility == 1
				? OperationsS10.VCVTS_D
				: OperationsA10.ZCVTS_T)//
	...
	public final String toCode() {
		...
		return (this.visibility == 2
			...
			: this.visibility == 1
				? OperationsS10.VCVTS_D
				: OperationsA10.ZCVTS_T) + "\t1\tV [" + this.argumentA + "]  ->S;";
	}
