 ...
 *
 */
public final class TKV_FSTORE_BA_VC_S extends TokenValue {
	 ...
	 * @param argumentB
	 */
	public TKV_FSTORE_BA_VC_S(final BasePrimitiveString argumentA, final TokenInstruction argumentB) {
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (modifierA == AA0RB) {
			if (this.argumentB.toPreferStackResult()) {
				...
				assembly.addInstruction(OperationsA2X.XFSTORE_D.instruction(this.argumentA, null, AE21POP, 0, store));
				return;
			}
			this.argumentB.toAssembly(assembly, null, null, ResultHandler.FA_BNN_NXT);
		}

		assembly.addInstruction(OperationsA2X.XFSTORE_D.instruction(this.argumentA, null, modifierA, 0, store));
	}

	@Override
	public final String toCode() {
		
		return "FSTORE\t2\tVC->S\tCONST('" + this.argumentA + " = " + this.argumentB + "');";
	}
}
