...
final class TKO_MMOD_T_BA_SS_S extends TokenOperator {

	public static final TokenOperator INSTANCE = new TKO_MMOD_T_BA_SS_S();

	private TKO_MMOD_T_BA_SS_S() {
		// ignore
	}
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (store == ResultHandler.FA_BNN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MMOD_T_BA_SS_V);
				return;
			}
		} else //
		...
		assembly.addInstruction(OperationsA2X.XMMOD_T.instruction(argumentA, argumentB, 0, store));
	}
	...
	public final TokenInstruction toExecDetachableResult() {
		...
		return TKO_MMOD_D_BA_SS_S.INSTANCE;
	}
