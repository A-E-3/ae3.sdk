...
final class TKO_MSHRU_N_BA_SS_S extends TokenOperator {
	
	public static final TokenOperator INSTANCE = new TKO_MSHRU_N_BA_SS_S();

	private TKO_MSHRU_N_BA_SS_S() {
		// ignore
	}
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (store == ResultHandler.FA_BNN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MSHRU_N_BA_SS_V);
				return;
			}
		} else //
		if (store == ResultHandler.FB_BSN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MSHRU_N_BA_SS_S);
				return;
			}
		}

		assembly.addInstruction(OperationsS2X.VMBSHRU_N.instruction(argumentA, argumentB, 0, store));
	}

	@Override
	public final String toCode() {
		
		return OperationsS2X.VMBSHRU_N + "\t2\tSS ->S;";
	}
	...
	public TokenInstruction toStackValue(final ProgramAssembly assembly, final TokenInstruction argumentA, final TokenInstruction argumentB, final boolean sideEffectsOnly) {
		...
		if (constantB != null) {
			return new TKV_ZMSHRU(argumentA, constantB.baseToJavaInteger(), 2);
		}
