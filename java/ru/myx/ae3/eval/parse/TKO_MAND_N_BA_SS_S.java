...
final class TKO_MAND_N_BA_SS_S extends TokenOperator {
	
	public static final TokenOperator INSTANCE = new TKO_MAND_N_BA_SS_S();

	private TKO_MAND_N_BA_SS_S() {
		// ignore
	}
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (store == ResultHandler.FA_BNN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MAND_N_BA_SS_V);
				return;
			}
		} else //
		if (store == ResultHandler.FB_BSN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MAND_N_BA_SS_S);
				return;
			}
		}

		assembly.addInstruction(OperationsS2X.VMBAND_N.instruction(argumentA, argumentB, 0, store));
	}

	@Override
	public final String toCode() {
		
		return OperationsS2X.VMBAND_N + "\t2\tSS ->S;";
	}
	...
	public TokenInstruction toStackValue(final ProgramAssembly assembly, final TokenInstruction argumentA, final TokenInstruction argumentB, final boolean sideEffectsOnly) {
		...
		if (constantB != null && constantA == null) {
			...
			if (intB >= InstructionA10.CNST_MIN && intB <= InstructionA10.CNST_MAX) {
				return new TKV_ZMAND(argumentA, intB, 2);
			}
		}
		if (constantA != null && constantB == null) {
			...
			if (intA >= InstructionA10.CNST_MIN && intA <= InstructionA10.CNST_MAX) {
				return new TKV_ZMAND(argumentB, intA, 2);
			}
		}
