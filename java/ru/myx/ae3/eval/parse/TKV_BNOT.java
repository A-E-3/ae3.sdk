...
final class TKV_BNOT extends TokenValue {
	...
	public InstructionEditable toConditionalSkipEditable(final ProgramAssembly assembly, final int start, final TokenInstruction.ConditionType compare, final ResultHandler store) {
		...
		if (modifierA != ModifierArguments.AA0RB) {
			/* inverted order */
			final InstructionEditable editable = compare.inverse().createEditable(modifierA, store);
			assembly.addInstruction(editable);
			return editable;
		}
		...
		/* inverted order */
		final InstructionEditable editable = compare.inverse().createEditable(direct, store);
		assembly.addInstruction(editable);
		return editable;
	}
	...
	public void toConditionalSkipSingleton(final ProgramAssembly assembly, final TokenInstruction.ConditionType compare, final int constant, final ResultHandler store) {
		...
		if (modifierA != ModifierArguments.AA0RB) {
			/* inverted order */
			assembly.addInstruction(compare.inverse().createSingleton(modifierA, constant, store));
			return;
		}
		...
		/* inverted order */
		assembly.addInstruction(compare.inverse().createSinglton(direct, constant, store));
	}
}
