...
final class TKO_MADD_N_BA_SS_S extends TokenOperator {
	
	public static final TokenOperator INSTANCE = new TKO_MADD_N_BA_SS_S();

	private TKO_MADD_N_BA_SS_S() {
		//
	}
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (store == ResultHandler.FA_BNN_NXT) {
			if (argumentA == ModifierArguments.AE21POP) {
				if (argumentA == argumentB) {
					assembly.addInstruction(Instructions.INSTR_MADD_N_BA_SS_V);
					return;
				}
				if (argumentB == ModifierArguments.AA0RB) {
					assembly.addInstruction(Instructions.INSTR_MADD_N_BA_RS_V);
					return;
				}
			}
		} else //
		if (store == ResultHandler.FB_BSN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MADD_N_BA_SS_S);
				return;
			}
		}

		assembly.addInstruction(OperationsS2X.VMADD_N.instruction(argumentA, argumentB, 0, store));
	}

	@Override
	public final String toCode() {
		
		return OperationsS2X.VMADD_N + "\t2\tSS ->S;";
	}
	...
	public TokenInstruction toStackValue(final ProgramAssembly assembly, final TokenInstruction argumentA, final TokenInstruction argumentB, final boolean sideEffectsOnly) {
		...
		if (constantA != null) {
			if (sideEffectsOnly && constantA.baseIsPrimitive()) {
				 ...
				 */
				return argumentB.toExecNativeResult();
			}
			if (constantB != null) {
		...
		switch (argumentA.getResultType()) {
			case STRING :
				return new TKV_MADDS_BA_VV_S(argumentA, argumentB, 2);
			case BOOLEAN :
			case INTEGER :
			case NUMBER :
				switch (argumentB.getResultType()) {
					...
					case INTEGER :
					case NUMBER :
						return new TKV_MADDN_BA_VV_S(argumentA, argumentB, 2);
					default :
						return super.toStackValue(assembly, argumentA, argumentB, sideEffectsOnly);
				}
			default :
