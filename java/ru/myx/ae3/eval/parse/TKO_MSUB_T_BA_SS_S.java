...
final class TKO_MSUB_T_BA_SS_S extends TokenOperator {

	public static final TokenOperator INSTANCE = new TKO_MSUB_T_BA_SS_S();

	private TKO_MSUB_T_BA_SS_S() {
		// ignore
	}
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (store == ResultHandler.FA_BNN_NXT) {
			if (argumentA == ModifierArguments.AE21POP) {
				if (argumentA == argumentB) {
					assembly.addInstruction(Instructions.INSTR_MSUB_T_BA_SS_V);
					return;
				}
				if (argumentB == ModifierArguments.AA0RB) {
					assembly.addInstruction(Instructions.INSTR_MSUB_T_BA_RS_V);
					return;
				}
			}
		} else //
		if (store == ResultHandler.FB_BSN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MSUB_D_BA_SS_S);
				return;
			}
		}

		assembly.addInstruction(OperationsA2X.XMSUB_T.instruction(argumentA, argumentB, 0, store));
	}

	@Override
	public final String toCode() {

		return OperationsA2X.XMSUB_T + "\t2\tSS ->S;";
	}

	@Override
	public TokenInstruction toExecDetachableResult() {

		return TKO_MSUB_D_BA_SS_S.INSTANCE;
	}

	@Override
	public TokenInstruction toExecNativeResult() {

		return TKO_MSUB_N_BA_SS_S.INSTANCE;
	}
}
