...
final class TKO_ACALLV_BA_VS_S extends TokenOperator {
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		final TokenInstruction callPropertyName = this.callPropertyName;
		final ModifierArgument modifierB = callPropertyName.toDirectModifier();
		if (modifierB == ModifierArguments.AA0RB) {
			assert argumentA != ModifierArguments.AA0RB;
			callPropertyName.toAssembly(assembly, null, null, ResultHandler.FA_BNN_NXT);
		}
		assembly.addInstruction(
				(callPropertyName.getResultType() == InstructionResult.STRING
