 ...
 *         Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments */
public final class TKV_ASSIGN_ELOA extends TokenValue {
	...
	/** @param reference
	 * @param value */
	public TKV_ASSIGN_ELOA(final TokenInstruction reference, final TokenInstruction value) {
			...
			if (onlyEffects != null) {
				assembly.addInstruction(
						modifierReferenced == ModifierArguments.AA0RB
							...
							: OperationsA11.XESKIP0A_P.instruction(modifierReferenced, 0, onlyEffects));
			}
			final InstructionEditable skip = modifierReferenced == ModifierArguments.AA0RB
				? OperationsA01.XESKIPRB1_P.instructionCreate(0, direct)
				: OperationsA11.XESKIP1A_P.instructionCreate(modifierReferenced, 0, direct);
			assembly.addInstruction(skip);
			final int rightStart = assembly.size();
			final ModifierArgument modifierValue = this.tokenRight.toDirectModifier();
			if (modifierValue == ModifierArguments.AA0RB) {
	...
	public String toCode() {
		...
		return "ASSIGN_ELOA [" + this.tokenLeft + " ||= " + this.tokenRight + "];";
	}
}
