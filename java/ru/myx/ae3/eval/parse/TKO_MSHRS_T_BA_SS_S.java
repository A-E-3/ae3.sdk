...
final class TKO_MSHRS_T_BA_SS_S extends TokenOperator {
	
	public static final TokenOperator INSTANCE = new TKO_MSHRS_T_BA_SS_S();

	private TKO_MSHRS_T_BA_SS_S() {
		// ignore
	}
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (store == ResultHandler.FA_BNN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MSHRS_T_BA_SS_V);
				return;
			}
		} else //
		if (store == ResultHandler.FB_BSN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MSHRS_D_BA_SS_S);
				return;
			}
		}

		assembly.addInstruction(OperationsA2X.XMBSHRS_T.instruction(argumentA, argumentB, 0, store));
	}

	@Override
	public final String toCode() {
		
		return OperationsA2X.XMBSHRS_T + "\t2\tSS ->S;";
	}

	@Override
	public TokenInstruction toExecDetachableResult() {
		
		return TKO_MSHRS_D_BA_SS_S.INSTANCE;
	}

	@Override
	public TokenInstruction toExecNativeResult() {
		
		return TKO_MSHRS_N_BA_SS_S.INSTANCE;
	}
	...
	public TokenInstruction toStackValue(final ProgramAssembly assembly, final TokenInstruction argumentA, final TokenInstruction argumentB, final boolean sideEffectsOnly) {
		...
		if (constantB != null) {
			return new TKV_ZMSHRS(argumentA, constantB.baseToJavaInteger(), 0);
		}
