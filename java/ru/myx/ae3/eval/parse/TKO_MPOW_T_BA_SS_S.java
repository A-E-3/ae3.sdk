...
final class TKO_MPOW_T_BA_SS_S extends TokenOperator {

	public static final TokenOperator INSTANCE = new TKO_MPOW_T_BA_SS_S();

	private TKO_MPOW_T_BA_SS_S() {
	...
	public void toAssembly(final ProgramAssembly assembly, final ModifierArgument argumentA, final ModifierArgument argumentB, final ResultHandlerBasic store) {
		...
		if (store == ResultHandler.FA_BNN_NXT) {
			if (argumentA == argumentB && argumentA == ModifierArguments.AE21POP) {
				assembly.addInstruction(Instructions.INSTR_MPOW_T_BA_SS_V);
				return;
			}
		} else //
		...
		assembly.addInstruction(OperationsA2X.XMPOW_T.instruction(argumentA, argumentB, 0, store));
	}

	@Override
	public final String toCode() {

		return OperationsA2X.XMPOW_T + "\t2\tSS ->S;";
	}

	@Override
	public TokenInstruction toExecDetachableResult() {

		return TKO_MPOW_D_BA_SS_S.INSTANCE;
	}
