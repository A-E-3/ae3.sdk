 ...
 * @param <T> */
@ReflectionIgnore
public abstract class AbstractServeRequestMutable<T extends ServeRequestEditable<T>> extends AbstractMessageMutable<T> implements ServeRequestEditable<T> {
	...
	protected AbstractServeRequestMutable(final String eventTypeId, final String verb, final BaseObject attributes) {

		super(attributes);
		this.eventTypeId = eventTypeId;
		this.verbOriginal = this.verb = verb;
	}
	...
	public Iterator<String> baseKeysOwn() {
		...
		return AbstractServeRequestMutable.KEYS.baseKeysOwn();
	}
	
	@Override
	public Iterator<? extends BasePrimitive<?>> baseKeysOwnPrimitive() {
		
		return AbstractServeRequestMutable.KEYS.baseKeysOwnPrimitive();
	}
	...
	/** Tries to parse an attribute "X-Session-Id". */
	@Override
	public String getSessionID() {
		
		return this.sessionID != null
			? this.sessionID
			: (this.sessionID = Base.getString(this.getAttributes(), "X-Session-Id", null));
	}
	...
	public ReplyAnswer toSecureChannel() {
		...
		if ("true".equals(Base.getString(this.attributes, "Secure", "false"))) {
			return null;
		}
		return Reply.string(this.getEventTypeId() + ":ER-TSC", this, "Access cannot be granted. Secure interface is unavailable but required!").setCode(Reply.CD_DENIED);
	}
	...
	public ReplyAnswer toUnSecureChannel() {
		...
		if ("false".equals(Base.getString(this.attributes, "Secure", "true"))) {
			return null;
		}
		return Reply.string(this.getEventTypeId() + ":ER-TUSC", this, "Access cannot be granted. UnSecure interface is unavailable but required!").setCode(Reply.CD_DENIED);
	}
