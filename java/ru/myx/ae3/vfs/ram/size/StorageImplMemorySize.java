...
/** @author myx */
public final class StorageImplMemorySize //
		implements
			ArsStorage<RecordMemory, ReferenceMemory, ArrayMemory> //
{

	static Value<ArrayMemory> getLinksRangeImpl(final Map<RecordMemoryKey, ReferenceMemory> source,
	...
	public StorageImplMemorySize(final String name) {
		...
		final Guid guid = Guid.forString(
				name == null
					...
					: name);
		this.rootReference = new ReferenceMemory(null, new RecordMemoryPrimitive(guid), TreeLinkType.NO_REFERENCE, System.currentTimeMillis(), new RecordMemoryCollection());
	}
	...
	public RecordMemory createPrimitiveTemplate(final Guid guid) {
		...
		/** base objects are more compact cause primitives are mostly stored as no duplicates in
		 * memory. */
