...
class HashMapPrimitiveIdentity<K, V> {
	...
	static final class Entry<K, V> {
		 ...
		 * Create new entry.
		 */
		Entry(final K k, final V v, final Entry<K, V> n) {
			this.value = v;
			this.next = n;
			this.key = k;
		}
	...
	HashMapPrimitiveIdentity() {
		this.loadFactor = HashMapPrimitiveIdentity.DEFAULT_LOAD_FACTOR;
		this.threshold = (int) (HashMapPrimitiveIdentity.DEFAULT_INITIAL_CAPACITY * HashMapPrimitiveIdentity.DEFAULT_LOAD_FACTOR);
		this.table = new Entry[HashMapPrimitiveIdentity.DEFAULT_INITIAL_CAPACITY];
	}
	 ...
	 */
	HashMapPrimitiveIdentity(final int initialCapacity) {
		this( initialCapacity, HashMapPrimitiveIdentity.DEFAULT_LOAD_FACTOR );
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	HashMapPrimitiveIdentity(int initialCapacity, final float loadFactor) {
		if (initialCapacity < 0) {
			throw new IllegalArgumentException( "Illegal initial capacity: " + initialCapacity );
		}
		if (initialCapacity > HashMapPrimitiveIdentity.MAXIMUM_CAPACITY) {
			initialCapacity = HashMapPrimitiveIdentity.MAXIMUM_CAPACITY;
		}
	...
	public final V get(final K key) {
		for (Entry<K, V> e = this.table[System.identityHashCode( key ) & this.table.length - 1]; e != null; e = e.next) {
			if (e.key == key) {
				return e.value;
			}
		}
	...
	private final Entry<K, V> removeEntryForKey(final int hash, final K key) {
		final int i = hash & this.table.length - 1;
		Entry<K, V> prev = this.table[i];
		Entry<K, V> e = prev;
	...
	private final void resize(final int newCapacity) {
		...
		if (oldCapacity == HashMapPrimitiveIdentity.MAXIMUM_CAPACITY) {
