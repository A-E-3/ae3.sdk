...
public abstract class AbstractCharacterConsole extends AbstractConsole {
	...
	public Value<Boolean> readBoolean(final String title, final boolean defaultValue) {
		...
		return "yes".equals(this.readString(title + "(yes/no)?", defaultValue
			? "yes"
			: "no"))
				? BaseObject.TRUE
				: BaseObject.FALSE;
	...
	public Value<Number> readInteger(final String title, final int defaultValue) {
		...
		return Base.forInteger(Integer.parseInt(this.readString(
				title, //
	...
	public final void sendMessage(final String message) {
		...
		synchronized (this) {
			this.clearProgress();
			this.sendMessageImpl(message == null
				? "null"
				: message);
		}
		this.flush();
	...
	public final void sendMessage(final String title, final String text) {
		...
		synchronized (this) {
			this.clearProgress();
			if (text == null) {
				this.sendMessageImpl(title == null
					? "null"
					: title);
			} else {
				this.sendMessageImpl(title == null
	...
	public final void sendProgress(final String string) {
		...
		if (string != null && string.length() > 0) {
			...
			final String checkProgress = AbstractCharacterConsole.PROGRESS_UNDEFINED_STATES.get(oldProgress == null
				? ""
				: oldProgress);
