 ...
 */
/** @author myx */
public final class ExecCachedInstance extends BaseFunctionAbstract
		implements
			ExecCallableBase.ExecStoreX,
			ExecCallableJava.NativeEX,
			CreationHandlerObject<ExecProcess, BaseObject> {
	...
	public BaseObject callNEX(final ExecProcess ctx, final BaseObject instance, final BaseArray arguments) {
		...
		if (instance == null) {
			throw new IllegalArgumentException("Instance methods are not accessible for null references!");
		}
		final String guid;
		{
			final BaseObject guidObject = instance.baseGet("guid", BaseObject.UNDEFINED);
			assert guidObject != null : "NULL java value";
			guid = guidObject.baseToBoolean() == BaseObject.TRUE
				? guidObject.baseToJavaString()
		...
		final String method;
		{
			...
			unique.append(','); // for instance methods
		...
		if (Report.MODE_DEVEL) {
			Report.info("TN_METHOD/CACHE/INSTANCE", "step1: " + method);
		}
		{
			final int ri0ASP = ctx.ri0ASP;
			ctx.vmFrameEntryExCall(true, instance, this, arguments, ResultHandler.FA_BNN_NXT);
	...
	public ExecStateCode execCallPrepare(final ExecProcess ctx, final BaseObject instance, final ResultHandler store, final boolean inline, final BaseArray arguments) {
		...
		if (instance == null) {
			return ctx.vmRaise("Instance methods are not accessible for null references!");
		}
		final String guid;
		{
			final BaseObject guidObject = instance.baseGet("guid", BaseObject.UNDEFINED);
	...
	public final String toString() {
		...
		return "CACHE-MTHD(I), key=" + this.method;
	}
}
