...
public interface TokenInstruction extends Instruction {
	...
	/**
	 * Result type when result count is 1. Null when result type is unknown.
	...
	default ModifierArgument toConstantModifier() {
		...
		/**
		 * Check code (do not uncomment all such code at once, could cause
		 * deadly recursions):<code>
	 ...
	 * @throws Evaluate.CompilationException
	 */
	default InstructionEditable toBooleanConditionalSkip(final ProgramAssembly assembly, final int start, final boolean compare, final boolean statement, final ResultHandler store)
			throws Evaluate.CompilationException {
	...
	/**
	 * Should return instruction producing same results from same arguments but
	 * definitely providing detach-able (not depending on context state) result
	 * in r7RR register (if any).
	 *
	 * Without calling this there is no guarantee that results would be valid
	 * for the next instruction.
	 *
	 * Every instruction using internal context's accumulators for results
	 * should override this method and provide means for receiving actually
	 * detach-able results.
	 *
	 * @return
	 */
	...
	/**
	 * Should return instruction producing same results from same arguments but
	 * definitely providing native (not depending on context state, not wrapped)
	 * result in r7RR register (if any).
	 *
	 * Without calling this there is no guarantee that results would be valid
	 * for the next instruction.
	 *
	...
	/**
	 * when isAccessReference, could be NULL when object is not known within a
	 * token and read from operand stack
	 *
	 * @return
	 */
	...
	/**
	 * when isAccessReference, could be NULL when property is not known within a
	 * token and read from operand stack
	 *
	 * @return
	 */
	 ...
	 *            - true if only code producing side effects is required
	 *            (optional), do not return NULL, return constant instead.
	 * @return
	 * @throws Evaluate.CompilationException
	 */
