...
public class SyntaxQuery extends Syntax {
	 ...
	 * @return string
	 */
	public static String filterToWhere(
		...
		for (final Object current : parsed) {
			...
			if (current instanceof OneCondition) {
				...
				final Function<OneCondition, OneCondition> replacement = Convert.Any.toAny( Convert.MapEntry
						.toObject( replacementConditions, conditionOriginal.getField(), null ) );
				...
				if ("=".equals( operand )) {
					if (Math.max( value.indexOf( '*' ), value.indexOf( '?' ) ) >= 0) {
						buffer.append( " LIKE '" ).append( value.replace( '*', '%' ).replace( '?', '_' ) )
								.append( '\'' );
					} else {
	...
	public static String filterToWhere(
		...
		return query == null
				? null
				: SyntaxQuery.filterToWhere( SyntaxQuery.parseFilter( defaultField,
						defaultCondition,
						replacementFields,
	 ...
	 * @return list
	 */
	public static List<Object> parseFilter(
			final String defaultField,
		...
		for (final String token : tokens) {
			...
			if (field != null) {
				...
				final OneCondition conditionOriginal = new OneConditionSimple( exact
						? true
						: field == defaultField && value.length() > 2
								? false
								: true, Convert.MapEntry.toString( replacementFields, field, field ), operand, value );
				final Function<OneCondition, OneCondition> replacement = Convert.Any.toAny( Convert.MapEntry
						.toObject( replacementConditions, conditionOriginal.getField(), null ) );
				final OneCondition condition;
				if (replacement == null) {
	 ...
	 * @return list
	 */
	public static List<Object> parseOrder(
			final List<Object> target,
			final Map<?, ?> replacementOrder,
